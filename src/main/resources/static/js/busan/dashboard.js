...
var loop = function loop() {
  var rad = (sin(frame / 1400) * PI) / 800;
  var rad2 = (sin(frame / 700) * PI) / 100;
  frame++;
  ...
  vertices.forEach(function (vertex, i) {
    ...
    var wave =
      cos(frame / 45 + x / 50) -
      sin(frame / 20 + z / 50) +
      sin(frame / 30 + (z * x) / 440000);
    var y = vertex[1] + wave * waveSize;
    var a = Math.max(0, 1 - Math.sqrt(Math.pow(x, 2) + Math.pow(z, 2)) / depth);
    var tx, ty, tz;
    y -= oceanHeight;
