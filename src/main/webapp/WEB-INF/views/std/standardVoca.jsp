		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" summary="표준단어 조회"> <!-- 표준단어조회 -->
            ...
            <tbody>          
       				...
       				<td>
				    	<select id="stndAsrt" name="stndAsrt" class="wd300">
				        	<option value="all">전체</option>
	 		      			<!--<option value="BD">부산형데이터</option>
				      		<option value="DS">데이터셋표준</option>-->
				      </select>
      				</td>
                    <th scope="row"><label for="stwdLnm">표준단어명</label></th> <!-- 표준단어명 -->
                    <td><input type="text" id="stwdLnm" name="stwdLnm" class="wd200 ui-autocomplete-input" autocomplete="off"><span role="status" aria-live="polite" class="ui-helper-hidden-accessible"></span></td>
                    <th scope="row"><label for="engMean">영문의미</label></th> <!-- 영문의미 -->
                    <td><input type="text" id="engMean" name="engMean" class="wd200"></td>
                    <th scope="row"><label for="cchNm">한자</label></th> <!-- 한자 -->
                ...
                <tr>
                    ...
                    <th>조회기간</th> <!-- 조회기간 -->
				 	<td><input id="searchBgnDe" name="searchBgnDe" type="text" class="wd80 hasDatepicker" value=""><img class="ui-datepicker-trigger" src="resources/images/icon-calendar.png" alt="날짜선택" title="날짜선택">  - <input id="searchEndDe" name="searchEndDe" type="text" class="wd80 hasDatepicker" value=""><img class="ui-datepicker-trigger" src="resources/images/icon-calendar.png" alt="날짜선택" title="날짜선택"></td>
	    			<th scope="row"><label for="orgDs">출처구분</label></th> <!-- 출처구분 -->
                    <td><input type="text" id="orgDs" name="orgDs" class="wd95p"></td>
                    <th scope="row"><label for="objDescn">설명</label></th> <!-- 설명 -->
                    <td><input type="text" id="objDescn" name="objDescn" class="wd95p"></td>
                </tr>
...
<script type="text/javascript">
    ...
    document.addEventListener("DOMContentLoaded", function(){
	    ...
	    grid = new tui.Grid({
	        ...
	        [
	            ...
	            {header:'표준단어논리명', name:'stdVocaLgcNm', align:'center' },
	            {header:'표준단어물리명', name:'stdVocaPhysNm', align:'center' },
	            {header:'영문의미', name:'engNm', align:'center' },
	            {header:'한자명', name:'chncrtNm', align:'center' },
	            {header:'출처구분', name:'srcSeNm', align:'center' },
    ...
    async function getStdClsfCd() {
    	  try {
    	    const response = await fetch("<c:url value='/std/getStdClsfCd'/>", {
    	      method: "GET", // 또는 'PUT'
    	      headers: {
    ...
    async function postJSON() {
    	  try {
    	    const response = await fetch("<c:url value='/std/standardVocaList2'/>", {
    	      method: "GET", // 또는 'PUT'
    	      headers: {
    	        "Content-Type": "application/json",
    	      }
    ...
    $("#btnSearch").click(async function(){
        ...
        // 표준 단어명
        const stwdLnm = document.getElementById("stwdLnm").value;
        console.log("표준단어명:", stwdLnm);
        // 영문의미
        const engMean = document.getElementById("engMean").value;
        console.log("영문의미:", engMean);
